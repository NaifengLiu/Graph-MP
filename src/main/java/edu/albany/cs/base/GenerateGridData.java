...
public class GenerateGridData {
	...
	public void generateGridDataWithNoise(int numTrueNodes, int NumOfNodes, double alpha, double noiseLevel,
		...
		for (Edge e : treEdges) {
			weight[e.i] = rand.nextDouble() * (1.0D * (new Random().nextInt(20) + 20.0D)) + 5.0D;
			weight[e.j] = rand.nextDouble() * (1.0D * (new Random().nextInt(20) + 20.0D)) + 5.0D;
			if (!ArrayUtils.contains(trueNodes, e.i)) {
				trueNodes = ArrayUtils.add(trueNodes, e.i);
			}
			if (!ArrayUtils.contains(trueNodes, e.j)) {
		...
		APDMInputFormat.generateAPDMFile(usedAlgorithm, dataSource, g.edges, weight, null, null, treEdges,
				outPutFileName);
	}
	...
	public void generateSingleCase() throws IOException {
		...
		String outputFileName = "data/GridData/APDM-GridData-" + graphSize + "_noise_" + noiseLevel + "_trueSubSize_"
				+ numOfTrueNodes + "_4.txt";
